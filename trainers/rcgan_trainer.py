...
class rcGANTrainer():
    def __init__(self, *args, **kwargs):
        ...
        (self.lambda_gp, self.lambda_ct) = kwargs.pop('lambdas', (0., 0.))
    ...
    def update(self):
        ...
        for i in range(self.num_critic):
            ...
            out_dis, out_feat = netD(x_real, y_noisy)
            if self.gan_loss == 'wgan':
            ...
            if self.lambda_ct > 0:
                out_dis_, out_feat_ = netD(x_real, y_noisy)
            ...
            y_clean = self.noise2clean(y_noisy)
            x_fake = netG(z, y_clean)
            out_dis, _ = netD(x_fake.detach(), y_noisy)
            if self.gan_loss == 'wgan':
            ...
            if self.lambda_gp > 0:
                d_loss_gp = self.lambda_gp * self.cond_gradient_penalty(
                    x_real, x_fake, y_noisy, netD)
